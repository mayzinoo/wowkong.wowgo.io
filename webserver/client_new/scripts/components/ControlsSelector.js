define([
    ...
    return React.createClass({
        ...
        render: function() {
            ...
            var showManual = this.state.selectedControl === 'manual';

            return D.div({ id: 'controls-selector-container' },
                D.div({ className: 'buttons-container noselect ' + this.props.controlsSize },
                    D.div({ className: 'button-holder' + (this.state.selectedControl === 'manual'? ' tab-active' : ''),
                     onClick: this._selectControl('manual',this.props.isMobileOrSmall) },
                        D.a(null,  'Manual' )
                    ),
                    D.div({ className: 'button-holder' + (this.state.selectedControl === 'strategy'? ' tab-active' : ''),
                     onClick: this._selectControl('strategy',this.props.isMobileOrSmall) },
                        D.a(null,  'Auto' )
                    )
                ),
