define([
    ...
    Engine.prototype.reducebalance = function(balance) {
        var new_balance = Number(balance).toFixed(8);
    ...
    Engine.prototype.readdbalance = function(balance) {
        var new_balance = Number(balance).toFixed(8);
        this.ws.emit('readd_balance', new_balance, function(error) {
     ...
     */
    Engine.prototype.bet = function(amount, autoCashOut, eth_value,  callback) {
        var old_autocacheout = 100;
        var old_bet = 10;  
        var payout = autoCashOut;
        var bet = amount;     
        var profit = Number((amount * autoCashOut) - bet).toFixed(5); 
        ...
        //if (this.gameState === 'STARTING')
            return this.doBet(old_bet, old_autocacheout, bet, payout, profit, eth_value, callback);
    ...
    Engine.prototype.doBet =  function(old_bet, old_autocacheout, bet, payout, profit, eth_value,  callback) {
        //console.log('dobet');
        //console.log(autoCashOut);
        var self = this;

        this.ws.emit('place_bet', old_bet, old_autocacheout, bet, payout, profit, eth_value, function(error) {
    ...
    AppDispatcher.register(function(payload) {
        ...
        switch(action.actionType) {
            ...
            case AppConstants.ActionTypes.PLACE_BET:
                EngineSingleton.bet(action.bet, action.cashOut, action.eth_value);
                break;
            ...
            case AppConstants.ActionTypes.END_GAME:
                EngineSingleton.readdbalance(action.balance);
                break;         
