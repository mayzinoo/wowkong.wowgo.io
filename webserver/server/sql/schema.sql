...
CREATE TABLE fundings (
    ...
    ethereum_withdrawal_txid text,
    ethereum_withdrawal_address text,
    created timestamp with time zone DEFAULT now() NOT NULL,
    description text,
    ethereum_deposit_txid text,
    withdrawal_id UUID,
    CONSTRAINT fundings_withdrawal_id_key UNIQUE (withdrawal_id)
);

ALTER TABLE ONLY fundings
    ADD CONSTRAINT fundings_user_id_ethereum_deposit_txid_key UNIQUE (user_id, ethereum_deposit_txid);
...
CREATE TABLE tips (
    ...
    tip_tx_id text NOT NULL,
    created timestamp with time zone DEFAULT now() NOT NULL
);


CREATE INDEX tips_from_user_id ON tips USING btree (from_user_id);
CREATE INDEX tips_to_user_id ON tips USING btree (to_user_id);
